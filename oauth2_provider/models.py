...
class AbstractApplication(models.Model):
    ...
    * :attr:`name` Friendly name for the Application
    ...
    redirect_uris = models.TextField(
        blank=True,
        verbose_name='重定向地址',
        help_text=_("允许的URI列表，以空格分隔"),
    )
    post_logout_redirect_uris = models.TextField(
        blank=True,
        verbose_name='注销后重定向地址',
        help_text=_("允许的注销后URI列表，以空格分隔"),
    )
    client_type = models.CharField(max_length=32, choices=CLIENT_TYPES, verbose_name='客户端类型')
    authorization_grant_type = models.CharField(max_length=32, choices=GRANT_TYPES, verbose_name='授权授予类型')
    ...
    skip_authorization = models.BooleanField(default=False, verbose_name='跳过授权')
